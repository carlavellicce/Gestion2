...
namespace GD_Final_Baternoa.Cliente
{
    public partial class ABMCLienteConsulta : Form
    {
        ...
        {
            int Cliente = int.Parse(textBoxCliente.Text);
            string sql = "SELECT *FROM Cliente join Domicilio on Cliente.idDomicilio = Domicilio.idDomicilio WHERE DNICliente = Cliente";
            con.Open();
